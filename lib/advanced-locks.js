...
class AdvancedLocks {
    ...
    async upgradeToExclusive(file, options = {}) {
        ...
        // Wait until the lockfile is fully released (no readers or writer, or file deleted)
        const fs = require('fs');
        ...
        while (true) {
            ...
            try {
                ...
                // If file is deleted or corrupted, break
                break;
            }
    ...
    getLockKey(file, options) {
        // Use the same key generation as getLockId for consistency
        return this.getLockId(file);
    }
