...
function acquireLock(file, options, callback) {
    ...
    mkdirp(dir, (err) => {
        ...
        // New: If mode is specified, use JSON lockfile
...
function acquireJsonLock(file, lockfilePath, options, callback) {
    ...
    fs.readFile(lockfilePath, 'utf8', (err, data) => {
        ...
        if (err) {
            ...
            try {
                ...
                return callback(Object.assign(new Error('Corrupted lockfile'), { code: 'ECORRUPT', file }));
            }
        }
        ...
        if (mode === 'shared') {
            if (lockData.writer) {
                return callback(Object.assign(new Error('Writer lock is held'), { code: 'ELOCKED', file }));
            }
        ...
        // Write the lockfile
        fs.writeFile(lockfilePath, JSON.stringify(lockData, null, 2), { flag: 'w' }, (err) => {
            if (err) { return callback(err); }

            // Provide a release function
            callback(null, (releasedCallback) => new Promise((resolve, reject) => {
                fs.readFile(lockfilePath, 'utf8', (err, data) => {
                    if (err) {
                    ...
                    } else if (lockData.writer === id) { lockData.writer = null; }
                    ...
                    fs.writeFile(lockfilePath, JSON.stringify(lockData, null, 2), { flag: 'w' }, () => {
                        if (releasedCallback) { releasedCallback(); }
                        resolve();
                    });
                });
