...
class PluginManager extends EventEmitter {
    ...
    registerPlugin(name, plugin) {
        ...
        // Remove Jest-specific logic from production code
        // (If you need Jest mocks, do it in test helpers, not here)
...
class LoggingPlugin {
    ...
    async log(level, message, data) {
        ...
        // Only await if logger.log is async
        if (this.options.format === 'json') {
            const result = this.logger.log(JSON.stringify(logEntry));
            if (result && typeof result.then === 'function') await result;
        } else {
            const result = this.logger.log(`[${logEntry.timestamp}] ${level.toUpperCase()}: ${message}`, data);
            if (result && typeof result.then === 'function') await result;
        }
    }
