...
class LoggingPlugin {
    ...
    logBeforeAcquire(context) {
        this.log('info', 'Lock acquisition started', context);
        return context;
    }

    logAfterAcquire(context) {
        this.log('info', 'Lock acquired successfully', context);
        return context;
    }

    logBeforeRelease(context) {
        this.log('info', 'Lock release started', context);
        return context;
    }

    logAfterRelease(context) {
        this.log('info', 'Lock released successfully', context);
        return context;
    }

    logError(context) {
        this.log('error', 'Lock operation failed', context);
        return context;
    }
    ...
    log(level, message, data) {
        ...
        if (this.options.format === 'json') {
            ...
            this.logger.log(`[${logEntry.timestamp}] ${level.toUpperCase()}: ${message}`, data);
        }
    }
