...
(isWindows ? it.skip : it)('should resolve symlinks by default', async () => {
    fs.writeFileSync(`${tmpDir}/foo`, '');
...
(isWindows ? it.skip : it)('should not resolve symlinks if realpath is false', async () => {
    fs.writeFileSync(`${tmpDir}/foo`, '');
    fs.symlinkSync(`${tmpDir}/foo`, `${tmpDir}/bar`);
...
it('should call the compromised function if ENOENT was detected when updating the lockfile mtime', async () => {
    ...
    const handleCompromised = async (err) => {
        ...
        expect(err.message.includes('ENOENT') || err.originalCode === 'ENOENT').toBe(true);
...
it('should throw an error by default when the lock is compromised', async () => {
    ...
    expect(stderr.includes('ECOMPROMISED') || stderr.includes('ENOENT')).toBe(true);
});
...
it('should allow floor\'ed second precision mtime', async () => {
    ...
    await new Promise(async (resolve, reject) => {
        await lockfile.lock(`${tmpDir}/foo`, {
            ...
            update: 1000,
            onCompromised: (err) => {
                if (err.code === 'ECOMPROMISED' || err.code === 'ERELEASED') {
                    resolve();
                } else {
                    reject(err);
                }
        ...
        await pDelay(3000);
        resolve(); // If not compromised, still resolve
    });
});
...
describe('shared/exclusive (reader-writer) locks', () => {
    const sharedFile = `${tmpDir}/shared-lock-test`;
    const lockfilePath = `${sharedFile}.lock`;
    ...
    it('should allow multiple shared locks', async () => {
        ...
        const release1 = await lockfile.lock(sharedFile, { mode: 'shared' });
        const release2 = await lockfile.lock(sharedFile, { mode: 'shared' });
    ...
    it('should not allow exclusive lock if shared locks are held', async () => {
        fs.writeFileSync(sharedFile, '');
        const release1 = await lockfile.lock(sharedFile, { mode: 'shared' });

        await expect(lockfile.lock(sharedFile, { mode: 'exclusive' })).rejects.toMatchObject({ code: 'ELOCKED' });
        await release1();
    });

    it('should not allow shared lock if exclusive lock is held', async () => {
        fs.writeFileSync(sharedFile, '');
        const release = await lockfile.lock(sharedFile, { mode: 'exclusive' });

        await expect(lockfile.lock(sharedFile, { mode: 'shared' })).rejects.toMatchObject({ code: 'ELOCKED' });
        await release();
    });

    it('should allow exclusive lock after all shared locks are released', async () => {
        fs.writeFileSync(sharedFile, '');
        const release1 = await lockfile.lock(sharedFile, { mode: 'shared' });
        const release2 = await lockfile.lock(sharedFile, { mode: 'shared' });
        ...
        await release1();
        await release2();
        const release3 = await lockfile.lock(sharedFile, { mode: 'exclusive' });

        expect(typeof release3).toBe('function');
        await release3();
    });

    it('should allow shared lock after exclusive lock is released', async () => {
        fs.writeFileSync(sharedFile, '');
        const release = await lockfile.lock(sharedFile, { mode: 'exclusive' });
